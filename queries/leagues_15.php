...
// Execute the query
//Most away matches lost
 	$query_main = "select * from (select awayteam,count(*) from GAMES G, SCORES S, RESULT R where G.ID = R.ID AND G.ID = S.ID AND G.DIV = R.DIV AND G.DIV = S.DIV AND ftr = 'H' and G.div = '$div' and ($like_statement) group by awayteam order by count(*) desc) where rownum <='$rank'";
...
<center>
<br>
<span>Following table shows the <u>top <?php echo $rank; ?></u> teams in <u><?php echo $selected_league ?></u> that lost to other teams on away grounds.<br>
The teams have been ranked based on number of away ground defeats in ascending order. The score represents number of such defeats.</span>
<br><br>
