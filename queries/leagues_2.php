...
// Execute the query
 	$query_main = "select * from (select awayteam,count(*) from GAMES G, SCORES S, RESULT R where G.ID = R.ID AND G.ID = S.ID AND G.DIV = R.DIV AND G.DIV = S.DIV AND ftr = 'A' and G.div = '$div' and ($like_statement) group by awayteam order by count(*) desc) where rownum <='$rank'";
