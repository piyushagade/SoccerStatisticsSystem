...
//Most games with goals only in 1st half by Home team
 	$query_main = "select * from (select hometeam, count(*) from GAMES G, SCORES S, RESULT R where G.ID = R.ID AND G.ID = S.ID AND G.DIV = R.DIV AND G.DIV = S.DIV AND hthg = fthg and hthg <> 0 and G.div = '$div' and ($like_statement) group by hometeam order by count(*) desc) where rownum <= '$rank'";
