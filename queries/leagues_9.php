...
// Execute the query
//Total games where teams who won away matches while losing in the first half
 	$query_main = "select awayteam,count(*) from GAMES G, SCORES S, RESULT R where G.ID = R.ID AND G.ID = S.ID AND G.DIV = R.DIV AND G.DIV = S.DIV AND G.div = '$div' and  htr='H' and ftr='A' and ($like_statement) group by awayteam order by count(*) desc";
