...
// Execute the query
//Most home games won
 	$query_main = "select * from (select hometeam, count(*) from GAMES G, SCORES S, RESULT R where G.ID = R.ID AND G.ID = S.ID AND G.DIV = R.DIV AND G.DIV = S.DIV AND ftr = 'H' and G.div = '$div' and ($like_statement) group by hometeam order by count(*) desc) where rownum <='$rank'";
