...
//Total goals by Home team scoring only in the 2nd half with 0 goals in 1st half
 	$query_main = "select * from (select hometeam,sum(fthg) from GAMES G, SCORES S, RESULT R where G.ID = R.ID AND G.ID = S.ID AND G.DIV = R.DIV AND G.DIV = S.DIV AND hthg = 0 and fthg > hthg and G.div = '$div' and ($like_statement) group by hometeam order by sum(fthg) desc) where rownum <= '$rank'";
...
<center>
<br>
<span>Following table shows the <u>top <?php echo $rank; ?></u> home teams in <u><?php echo $selected_league ?></u> that scored only in second half.<br>
The teams have been ranked based on number of goals in such games. The score represents number of such goals.</span>
<br><br>
